...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result){
    ...
    while (exp_result < 28 && !s21_decimal_is_zero(remainder) && !s21_decimal_is_full(tmp_result)) {
        ...
        if (s21_mul(remainder, ten, &remainder) == 1) {
            ...
            s21_mul(tmp_result, ten, &tmp_result) == 1;
            s21_add(tmp_result, quotient, &tmp_result);
            exp_result++;
        }
    }

    if ((exp_result == 28 || s21_decimal_is_full(tmp_result)) && !s21_decimal_is_zero(remainder)) {
